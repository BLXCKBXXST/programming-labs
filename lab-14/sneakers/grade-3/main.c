...
// Принимаем **head — адрес указателя, чтобы менять его если список пустой.
void PushBack(Node **head) {
    Node *node = CreateNode();

    if (*head == NULL) {
        *head = node; // список пустой — новый узел становится головой
        return;
    }

    // идём до конца и цепляем туда
    Node *cur = *head;
    while (cur->next != NULL) {
        cur = cur->next;
    }
...
// Печатает весь список в виде таблицы

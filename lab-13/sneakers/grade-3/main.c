...
void FillSneakers(Sneaker s[N]) {
    char *b[]={"Nike","Adidas","Puma"}; char *m[]={"Pro","Super","Ultra"};
    int p[3][3]={{200,350,500},{250,400,600},{300,450,700}};
    srand(time(NULL));
    for(int i=0;i<N;i++){int bi=rand()%3,mi=rand()%3;
        strcpy(s[i].brand,b[bi]); strcpy(s[i].model,m[mi]);
        s[i].size=36+rand()%9; s[i].price=p[bi][mi];}
}
